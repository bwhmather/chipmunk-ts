...
var frand_unit_circle = function () {
	var vv = cp.v(mersenne.rand_real() * 2 - 1, mersenne.rand_real() * 2 - 1);
	return (cp.vlengthsq(vv) < 1 ? vv : frand_unit_circle());
};

var add_circle = function (i, radius) {
	var mass = radius * radius / 25;
	var body = space.addBody(new cp.Body(mass, cp.momentForCircle(mass, 0, radius, cp.vzero)));
	//	cpBody *body = cpSpaceAddBody(space, cpBodyInit(&bodies[i], mass, cpMomentForCircle(mass, 0, radius, vzero)));
	body.p = cp.vmult(frand_unit_circle(), 180);


	var shape = space.addShape(new cp.CircleShape(body, radius, cp.vzero));
...
var add_box = function (i, size) {
	...
	var shape = space.addShape(new cp.BoxShape(body, size, size));
	shape.e = 0; shape.u = 0.9;
};
...
var add_hexagon = function (i, radius) {
	...
	for (var i = 0; i < 12; i += 2) {
		...
		var p = cp.vmult(v(Math.cos(angle), Math.sin(angle)), radius);
		hexagon[i] = p.x;
		hexagon[i + 1] = p.y;
	}

	var mass = radius * radius;
	var body = space.addBody(new cp.Body(mass, cp.momentForPoly(mass, hexagon, cp.vzero)));
	body.p = vmult(frand_unit_circle(), 180);

	var shape = space.addShape(new cp.PolyShape(body, hexagon, cp.vzero));
	shape.e = 0; shape.u = 0.9;
};
...
var setupSpace_simpleTerrain = function () {
	...
	for (var i = 0; i < (simple_terrain_verts.length - 1); i++) {
		...
		space.addShape(new cp.SegmentShape(
			space.staticBody,
			cp.vadd(a, offset),
			cp.vadd(b, offset),
			0,
...
var rand_size = function () {
	return Math.pow(1.5, cp.lerp(-1.5, 3.5, mersenne.rand_real()));
};
...
add_benchmark({ name: 'ComplexTerrainCircles 1000', ticks: 100 }, function () {
	...
	for (var i = 0; i < (complex_terrain_verts.length - 1); i++) {
		...
		space.addShape(new cp.SegmentShape(space.staticBody, vadd(a, offset), vadd(b, offset), 0));
	}

	for (var i = 0; i < 1000; i++) {
		var radius = 5;
		var mass = radius * radius;
		var body = space.addBody(new cp.Body(mass, cp.momentForCircle(mass, 0, radius, vzero)));
		body.p = cp.vadd(cp.vmult(frand_unit_circle(), 180), v(0, 300));

		var shape = space.addShape(new cp.CircleShape(body, radius, vzero));
		shape.e = 0; shape.u = 0;
	}
...
add_benchmark({ name: 'ComplexTerrainHexagons 1000', ticks: 50 }, function () {
	...
	for (var i = 0; i < 1000; i++) {
		var mass = radius * radius;
		var body = space.addBody(new cp.Body(mass, cp.momentForPoly(mass, hexagon, vzero)));
		body.p = cp.vadd(cp.vmult(frand_unit_circle(), 180), new cp.Vect(0, 300));
...
add_benchmark('BouncyTerrainCircles 500', function () {
	...
	for (var i = 0; i < 500; i++) {
		...
		var mass = radius * radius;
		var body = space.addBody(new cp.Body(mass, cp.momentForCircle(mass, 0, radius, cp.vzero)));
		body.p = cp.vadd(cp.vmult(frand_unit_circle(), 130), cp.vzero);
		var vv = cp.vmult(frand_unit_circle(), 50);
...
add_benchmark('BouncyTerrainHexagons 500', function () {
	...
	for (var i = 0; i < (bouncy_terrain_verts.length - 1); i++) {
		var a = bouncy_terrain_verts[i], b = bouncy_terrain_verts[i + 1];
		var shape = space.addShape(new cp.SegmentShape(space.staticBody, cp.vadd(a, offset), vadd(b, offset), 0));
		shape.e = 1;
	}
	...
	for (var i = 0; i < 500; i++) {
		var mass = radius * radius;
		var body = space.addBody(new cp.Body(mass, momentForPoly(mass, hexagon, cp.vzero)));
		body.p = cp.vadd(vmult(frand_unit_circle(), 130), vzero);
		var vv = cp.vmult(frand_unit_circle(), 50);
		body.vx = vv.x; body.vy = vv.y;

		var shape = space.addShape(new cp.PolyShape(body, hexagon, vzero));
		shape.e = 1;
	}
...
add_benchmark({ name: 'PyramidTopple', dt: 1 / 180, ticks: 400 }, function () {
	...
	for (var i = 0; i < n; i++) {
		for (var j = 0; j < (n - i); j++) {
			...
			if (j === 0) {
				add_domino(cp.vadd(offset, cp.v(0.5 * (WIDTH - HEIGHT), HEIGHT + WIDTH)), false);
			}

			if (j != n - i - 1) {
				add_domino(cp.vadd(offset, cp.v(HEIGHT * 0.75, (HEIGHT + 3 * WIDTH) / 2)), true);
			} else {
				add_domino(cp.vadd(offset, cp.v(0.5 * (HEIGHT - WIDTH), HEIGHT + WIDTH)), false);
			}
		}
	}

	// Add a circle to knock the dominoes down
	var body = space.addBody(new cp.Body(2, cp.momentForCircle(2, 0, 5, cp.v(0, 0))));
	body.setPos(v(65, 100));
	var shape = space.addShape(new cp.CircleShape(body, 5, cp.v(0, 0)));
	shape.setElasticity(0);
});
